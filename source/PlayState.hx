...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		if (FlxG.save.data.colour) healthBar.createFilledBar(dad.barColor, boyfriend.barColor);
		else if (!FlxG.save.data.colour) healthBar.createFilledBar(0xFFFF0000, 0xFF66FF33);
		...
		replayTxt = new FlxText(healthBarBG.x + healthBarBG.width / 2 - 85, healthBarBG.y + (PlayStateChangeables.useDownscroll ? 540 : -540), 0, "REPLAY", 20);
		replayTxt.setFormat(Paths.font("vcr.ttf"), 42, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE,FlxColor.BLACK);
		replayTxt.borderSize = 4;
		replayTxt.borderQuality = 2;
		replayTxt.scrollFactor.set();
		replayTxt.alpha = 0.47;

		botplayTxt = new FlxText(healthBarBG.x + healthBarBG.width / 2 - 85, healthBarBG.y + (PlayStateChangeables.useDownscroll ? 540 : -540), 0, "BOTPLAY", 20);
		...
		if(PlayStateChangeables.botPlay && !loadRep) add(botplayTxt);
		if(!PlayStateChangeables.botPlay && loadRep) add(replayTxt);
		if(FlxG.save.data.scoreText) add(scoreTxt);
